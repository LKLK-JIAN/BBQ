...
public class RingProgressBar extends View {
    ...
    //单位
    private float bftSize;
    //圆环宽度
    private float ringWidth;
    //最大值
    private int max;
    //进度值
    private float progress = (float) 0.0;
    //是否显示文字
    ...
    //圆心坐标
    private Point mCenterPaint;
    private float mRadiua;

    private String text;
    //bft
    private Paint mPaintBFT;
    ...
    public RingProgressBar(Context context, AttributeSet attrs, int defStyle) {
        ...
        text=mTypedArray.getString(R.styleable.RingProgressBar_present);
        ringColor = mTypedArray.getColor(R.styleable.RingProgressBar_ringColorring, Color.WHITE);
        unit = mTypedArray.getString(R.styleable.RingProgressBar_unittemp);
        ...
        textSize = mTypedArray.getDimension(R.styleable.RingProgressBar_textSize, 8);
        bftSize = mTypedArray.getDimension(R.styleable.RingProgressBar_bftSize, 16);
    ...
    private void updateArcPaint() {
        ...
        SweepGradient sweepGradient = new SweepGradient(mCenterPaint.x, mCenterPaint.y, mGradientColors, null);
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        centre = getWidth() / 2;
        radius = (int) (centre - ringWidth / 2)-20;
        //绘制外层圆
        drawCircle(canvas);
        //绘制进度条
        drawProgress(canvas);
    ...
    private void drawTextContent(Canvas canvas) {
        ...
        canvas.drawText(String.format("%.1f",progress) + unit, width/2, height/2, paint);
        //设置stroke的宽度
        mPaintBFT.setStrokeWidth(0);
        //设置文字的颜色
        mPaintBFT.setColor(textColor);
        //设置文字的大小
        mPaintBFT.setTextSize(bftSize);
        //设置文字的style
        mPaintBFT.setTypeface(Typeface.DEFAULT);
        mPaintBFT.setTextAlign(Paint.Align.CENTER);
        //float bftSize = mPaintBFT.measureText();
        canvas.drawText(text, width/2, height/4 , mPaintBFT);
    }
    ...
    private void drawProgress(Canvas canvas) {
        ...
        canvas.drawArc(fillOval, 0, 360, true, paint);
//    switch (style) {
//      case STROKE: {
//        paint.setStyle(Paint.Style.STROKE);
//        paint.setStrokeCap(Paint.Cap.ROUND);
...
//    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        //确定圆心的坐标
        mCenterPaint.x = getMeasuredWidth() / 2;
        mCenterPaint.y = getMeasuredHeight() / 2;
     ...
     */
    public synchronized void setProgress(float progress,String unit) {
        this.unit=unit;
